...
function setEventHandlerClick(handler, dom, tag, update) {
    ...
    if (!__clickHandlers[rootTagName]) __clickHandlers[rootTagName] = {};
  ...
  _$(rootTag).on(clickEvent, clickNode, function(e){
	...
	if (riotAttr){
        ...
        else if (riotAttr.match(/closest:/)) {
            $item = _$(this).closest(riotAttr.match(/closest:(.+)/)[1]);
        }
        ...
        for (var key in attrs) {
            var attr = attrs[key].nodeName;
            if (attr !== undefined) {
                var value = attrs[key].value;
                switch (attr) {
                case "riot-addclass":
                    $item.addClass(value.replace(/^\./g, ""));
                    break;
                case "riot-removeclass":
                    $item.removeClass(value.replace(/^\./g, ""));
                    break;
                case "riot-toggleclass":
                    $item.toggleClass(value.replace(/^\./g, ""));
                    break;
                }
            }
        }
