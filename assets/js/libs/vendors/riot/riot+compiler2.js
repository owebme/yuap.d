...
var RE_ORIGIN = /^.+?\/+[^\/]+/,
  ...
  window.clickEvent = doc && 'ontouchstart' in doc.documentElement ? 'tap' : 'click'; /** MY FIX */
 ...
 */
function setEventHandlerClick(handler, dom, tag, update) {
	...
	var _className = dom.className.replace(/\s/g, ".").replace(/\.?\{.+\}/g, "");

	if (__clickHandlers[tag.parent.root.tagName + "__" + _className]) return;

	// console.log(tag.parent.root.tagName + "__" + _className);
	// console.dir(tag);

	__clickHandlers[tag.parent.root.tagName + "__" + _className] = _className;

  _$(tag.parent.root).on(clickEvent, "." + _className, function(e){

	if (this.getAttribute("riot-item")){
		var $item = _$(this.getAttribute("riot-item"));
		if (this.getAttribute("riot-addClass")){
			$item.addClass(this.getAttribute("riot-addClass"));
		}
    ...
    if (this["_handler"].call(tag, e) !== true && !/radio|check/.test(dom.type)) {
      if (e.preventDefault) e.preventDefault()
      e.returnValue = false
    }

    if (!e.preventUpdate && update) {
		if (update === "all"){
...
function update(expressions, tag) {
  ...
  each(expressions, function(expr, i) {
	...
	if (attrName !== "class") remAttr(dom, attrName)

    /** MY FIX */

    // event handler
    if (isFunction(value)) {
		if (attrName === "onclickdelegate"){
			setEventHandlerClick(value, dom, tag, false)
		}
		else if (attrName === "onclickdelegateupdate"){
			setEventHandlerClick(value, dom, tag, true)
		}
		else if (attrName === "onclickdelegateupdateall"){
